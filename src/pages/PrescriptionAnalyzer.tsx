...
// Constants
const logoUrl = "https://i.postimg.cc/J0Rsp5YD/f9128c6e-ffe6-41a0-a729-14bec879bb41-removalai-preview.png";
...
You are an expert in analyzing prescription documents. Your task is to provide a detailed analysis of the prescription image uploaded:

1. Medication Details:
   - List each medication with its name (brand and generic if visible)
   - Dosage, frequency, and duration
   - Purpose/indication (if stated or clearly implied)
2. Prescriber Information: Hospital/clinic name, doctor name/credentials (without revealing full personal details)
3. Patient Instructions: Special instructions for taking medications
4. Plain Language Explanation: What each medication is typically used for
5. Common Side Effects: List potential common side effects for each medication
6. Generic Alternatives: Mention if generic versions exist
7. Quality Assessment: Note any illegible text or ambiguous instructions
8. Recommended Follow-up: Suggest appropriate next steps
...
ALWAYS include this disclaimer: "This analysis is provided for educational purposes only. Always consult with your healthcare provider before making any changes to your treatment plan."
`;
...
const PrescriptionAnalyzer: React.FC = () => {
    ...
    const analyzeDocument = async () => {
        ...
        try {
            const { base64Image, mimeType, error } = await preprocessImage(prescriptionFile);
            if (error) {
    ...
    const downloadResult = () => {
        ...
        a.href = url;
        a.download = `EarlyMed_Prescription_Analysis.txt`;
        a.click();
            ...
            <main className="flex-grow flex justify-center items-center px-4">
                <div className="glass-card p-8 w-full max-w-4xl">
                    <div className="flex justify-center mb-6">
                        <img src={logoUrl} alt="Prescription Analyzer Logo" className="h-32" />
                    </div>
                    <p className="text-center text-lg mb-8 text-purple-600 dark:text-purple-200">Unlock insights from your prescriptions with AI</p>

                    <div className="mb-6">
                        <label className="block text-center mb-4 text-purple-700 dark:text-purple-100">Upload Your Prescription</label>
                        <div className="glass-input flex items-center justify-center h-40 mb-4">
                            ...
                            >
                                <Camera className="h-6 w-6 mr-2" /> Upload Image
                            </button>
                        </div>
                        {prescriptionPreview && (
                            <div className="flex justify-center mb-4">
                                <img src={prescriptionPreview} alt="Preview" className="max-w-xs rounded-lg shadow-md" />
                            </div>
                        )}
                        {qualityMessage && (
                    ...
                    <button
                        ...
                        disabled={isAnalyzing || !prescriptionFile}
                    >
                        {isAnalyzing ? (
                    ...
                    {!analysisResult && (
                        <div className="glass-card p-6 mt-6">
                            ...
                            <ul className="list-disc list-inside text-purple-600 dark:text-purple-300">
                                ...
                                <li>Click "Analyze Prescription" to process.</li>
                                <li>Review the detailed analysis and download it.</li>
                            </ul>
                            <p className="text-center mt-4 text-purple-600 dark:text-purple-300">
                                <strong>Privacy Note:</strong> Your prescriptions are processed securely and not stored.
                            </p>
                        </div>
                    )}
                </div>
